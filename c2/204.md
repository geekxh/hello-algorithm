...
<center>**dp[i][j] : 表示包含第i行j列元素的最小路径和**</center>

<bra>

我们很容易想到可以自顶向下进行分析。并且，无论最后的路径是哪一条，它一定要经过最顶上的元素，即

[0,0]。所以我们需要对dp[0][0]进行初始化。

<center>**dp[0][0] = [0][0]位置所在的元素值**</center>
...
<center>**dp[i][j] = min(dp[i-1][j-1],dp[i-1][j]) + triangle[i][j]**</center>
...
<center>**dp[1][0] = triangle[1][0] + triangle[0][0]**</center>

<center>**dp[1][1] = triangle[1][1] + triangle[0][0]**</center>

<bra>

最后，我们只要找到**最后一行元素中，路径和最小的一个**，就是我们的答案。即：

<center>**l：dp数组长度**</center>

<center>**result = min(dp[l-1,0]，dp[l-1,1]，dp[l-1,2]....)**</center>
